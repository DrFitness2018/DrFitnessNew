...
const Trainers = ({ match }) => {
  return (
    <>
      <Row>
        <Colxx xxs="12">
          <Breadcrumb heading="Appoint A Fitness Trainer" match={match} />
      ...
      <Row>
        {Trainer.map((item) => {
            return (
              ...
              <UserCards mainTitle={item.label} image={item?.imgSrc} rating={item.star}
                details ={item.details} fee ={item.fee} />
            );
          })}
      </Row>
    </>
        ...
        <Colxx lg="4" md="6" sm="12" className="mb-4">
            ...
            >
              Fee: {props.fee} Rs
                  ...
                  <CardText className="text-muted text-small mb-4 text-center">
                    {props.details}
                  </CardText>
